...
library(corrplot)
pval_comp=cbind(ORA=ora_result[,c("ID", "pval")], CERNO=cerno_result[,"pval"], Z=z_result[,"pval"],
                S2N=s2n[,"pval"],  S2N_abs=s2n_abs[,"pval"],  
                LFC=lfc[,"pval"], LFC_abs=lfc_abs[,"pval"],
                PLAGE=plage_result[, "pval"])
...
```{r comparison}
compared <- data.frame(ORA=nrow(significant_ora), CERNO=nrow(significant_cerno), Z_transform=nrow(significant_z), PLAGE=nrow(significant_plage))
...
## Joint gene sets
```{r}
joint_names <- Reduce(intersect, list(significant_z$Title, significant_cerno$Title, significant_ora$Title, significant_plage$Title))
...
best_title <- KEGGhsa[rank_pathways[1]]$MODULES$Title
genes <- rownames(data) %in% KEGGhsa[rank_pathways[1]]$GENES$ID
x <- data[genes,]
x_umap <- umap(t(x))
...
df$SVD <- t(decomposition(data, KEGGhsa[rank_pathways[1]]))
labels <- gsub("d", "disease", gsub("c", "control", metaInfo$Group))
df$Group <- labels
...
fig <- fig %>% layout(title = best_title)
fig
```
